...
check_dependencies() {
    ...
    if command -v x86_64-w64-mingw32-gcc >/dev/null 2>&1; then
        ...
        echo "⚠️  MinGW-w64 not found - Windows builds disabled"
    ...
    if command -v wget >/dev/null 2>&1 && [ -f "./build_appimage.sh" ]; then
        ...
        echo "⚠️  AppImage tools not found - AppImage builds disabled"
...
create_appimage() {
    if [ "$APPIMAGE_BUILD_AVAILABLE" = false ]; then
        echo "⚠️  Skipping AppImage build - dependencies not met"
        return
    fi
...
build_windows() {
    if [ "$WINDOWS_BUILD_AVAILABLE" = false ]; then
        echo "⚠️  Skipping Windows build - MinGW-w64 not available"
        return
    fi
